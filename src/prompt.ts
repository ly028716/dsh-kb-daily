...
export interface SectionConfig {
  ...
  reportDir: string
  /** Model-tool names used by this vault's isolated tool namespace. */
  toolNames?: {
    listModified: string
    read: string
    readDiff: string
...
/** Stable system-prompt section identity for one vault. */
export function sectionName(vaultId?: string): string {
  return vaultId === undefined ? KB_SECTION_NAME : `kb-daily:${vaultId}:task`
}
...
export function sectionText(config: SectionConfig): string {
  const names = config.toolNames ?? {
    listModified: 'kb_list_modified',
    read: 'kb_read',
    readDiff: 'kb_read_diff',
    writeReport: 'kb_write_report',
  }
  return [
    'You are the knowledge-base daily analyst.',
    'Produce today\'s daily digest on request:',
    `1. Call ${names.listModified} to find notes changed today (vault-local date).`,
    `2. Read the changed notes with ${names.read} (vault-relative paths only).`,
    `If ${names.readDiff} is available and returns a usable diff, use it as optional change context; Git is not required and git_unavailable is non-fatal.`,
    `3. Write a concise Chinese Markdown report with ${names.writeReport}; it lands in ${config.reportDir}/YYYY-MM-DD.md.`,
    ...
    `Never include an absolute local filesystem path in the report; use only the vault-relative paths returned by ${names.listModified}.`,
